			...
			if(!empty($user_id)){
				...
				if(preg_match('/{' . $mypost->post_type . '_login_password_reset_link}/',$message) || preg_match('/{' . $mypost->post_type . '_passw_reset_link}/',$message)){
					...
					if(!empty($login_settings['login_page'])){
						...
						$args = array('emd_action' => 'rp', 'emd_key' => $emd_rp_key, 'emd_login' => rawurlencode($user_login));
						$args = apply_filters('emd_passw_reset_link_args',$args,$app,$pid);
						$emd_rp_link = add_query_arg($args,untrailingslashit($login_url));
						$message = str_replace('{' . $mypost->post_type . '_login_password_reset_link}', $emd_rp_link, $message);
						$message = str_replace('{' . $mypost->post_type . '_passw_reset_link}', $emd_rp_link, $message);
					}
				}
			}
		}
...
if (!function_exists('emd_check_userEmail')) {
	...
	function emd_check_userEmail() {
		...
		$pid = isset($_GET['pid']) ? (string) $_GET['pid'] : '';
		...
		if(!empty($ent_list[$post_type]['user_email_key']) && $email_key == $ent_list[$post_type]['user_email_key']){
			...
			if(!empty($user_data) && !empty($pid) && !empty($ent_list[$post_type]['user_key'])){
				$user_key = $ent_list[$post_type]['user_key'];
				$user_id = get_post_meta($pid,$user_key,true);
				if($user_id != $user_data->data->ID){
					$response = false;
				}
			}
...
if (!function_exists('emd_parse_set_filter')) {
	function emd_parse_set_filter($set){
		...
		foreach($set_list as $aset){
			...
			if($aset_arr[0] == 'misc' && $aset_arr[1] == 'post_id'){
				$ret_sets['post_id'] = $aset_arr[3];
			}
		}
...
if (!function_exists('emd_global_val')) {
	function emd_global_val($app,$key,$set_list=Array()){
		if(!empty($set_list) && isset($set_list[$key])){
			return $set_list[$key];
		}
...
if(!function_exists('emd_featured_img')){
	function emd_featured_img($type,$size,$app,$pid=''){
		...
		if($type == 'url'){
			...
			elseif(file_exists(constant(strtoupper($app) . '_PLUGIN_DIR') . 'assets/img/' . $post_type . '_default.svg')){
				return constant(strtoupper($app) . '_PLUGIN_URL') . 'assets/img/' . $post_type . '_default.svg';
			}
			else {
				return '';
			}
			...
			elseif(file_exists(constant(strtoupper($app) . '_PLUGIN_DIR') . 'assets/img/' . $post_type . '_default.svg')){
				$img_src = constant(strtoupper($app) . '_PLUGIN_URL') . 'assets/img/' . $post_type . '_default.svg';
